...
async function compressImageForUpload(file) {
  ...
  if (bestBlob.size > HARD_SEND_LIMIT_BYTES) {
    throw new Error(
      `La imagen optimizada sigue pesando ${formatBytes(bestBlob.size)}. Haz una captura más simple del documento.`
    );
  }

  const base = String(file.name || "documento").replace(/\.[^.]+$/, "");
  const optimizedFile = new File([bestBlob], `${base}-optimizado.jpg`, {
    type: "image/jpeg",
...
async function prepareUploadFile(file) {
  ...
  if (isPdfFile(file)) {
    if (file.size > HARD_SEND_LIMIT_BYTES) {
      throw new Error(
        `El PDF pesa ${formatBytes(file.size)}. Para evitar error de subida, sube una foto o captura clara del documento. Las fotos se optimizan automáticamente.`
      );
    }
...
async function postAnalyzeWithFallback(formData) {
  ...
  throw new Error(`No se pudo analizar el documento. Intentos: ${errors.join(" | ")}`);
}
  ...
  async function handleFileChange(selectedFile) {
    ...
    try {
      ...
      if (prepared.optimized) {
        setErr(
          `✅ Imagen optimizada automáticamente: ${formatBytes(prepared.originalSize)} → ${formatBytes(prepared.finalSize)}`
        );
      }
    } catch (e) {
      setFile(null);
            ...
            >
              ...
              >
                ...
                <p style={{ color: "#64748b", lineHeight: 1.5, marginBottom: 18 }}>
                  Sube una foto o captura de la multa. Si pesa mucho, la optimizamos automáticamente antes de enviarla.
                </p>
                ...
                >
                  {preparing ? "Optimizando…" : loading ? "Analizando…" : "Analizar multa gratis"}
                </button>
