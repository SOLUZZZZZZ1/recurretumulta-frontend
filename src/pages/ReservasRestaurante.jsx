...
export default function ReservasRestaurante() {
  ...
  async function guardar() {
    // validación simple (para no liarla al equipo)
  ...
  if (!autorizado) {
    return (
      <div style={{ padding: 40, maxWidth: 420, margin: "0 auto" }}>
        ...
        <p style={{ marginTop: 8, opacity: 0.85 }}>Introduce el PIN para acceder. Se guarda solo durante esta sesión.</p>
        <input
          type="password"
          placeholder="PIN"
          value={pinInput}
  ...
  return (
    <div style={{ padding: 20 }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 10, margin: "12px 0" }}>
        ...
        <button onClick={() => setShowCanceladas((v) => !v)}>{showCanceladas ? "Ocultar canceladas" : "Mostrar canceladas"}</button>
      </div>

      <div style={{ marginBottom: 10 }}>
        <strong>Pax totales visibles:</strong> {totalPax} &nbsp; | &nbsp; Pendientes: {contadores.pendientes} · Llegaron: {contadores.llegaron} · No show:{" "}
        {contadores.no_show} · Canceladas: {contadores.canceladas}
      </div>
        ...
        >
          ...
          >
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, marginTop: 14 }}>
              ...
              <button onClick={guardar} style={{ padding: "10px 12px", fontWeight: 800 }}>
                Guardar
              </button>
            </div>
          </div>
