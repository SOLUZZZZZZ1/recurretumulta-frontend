...
export default function OpsCaseDetailPro() {
  ...
  {(() => {
    const hasRecurso = documents.some(d =>
      (d.kind || "").includes("pdf") && !d.kind.includes("authorization")
    );

    const hasAutorizacion = documents.some(d =>
      (d.kind || "").includes("authorization")
    );

    const hasOriginal = documents.some(d =>
      (d.kind || "").includes("original")
    );

    const ready = hasRecurso && hasAutorizacion && hasOriginal;

    return (
      <div className="mt-2 space-y-2 text-sm">
        <div>{hasRecurso ? "✔" : "❌"} Recurso generado</div>
        <div>{hasAutorizacion ? "✔" : "❌"} Autorización</div>
        <div>{hasOriginal ? "✔" : "❌"} Multa original</div>

        <div className="mt-3">
          <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
            ready ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"
          }`}>
            {ready ? "LISTO PARA ENVIAR" : "INCOMPLETO"}
          </span>
        </div>
      </div>
    );
